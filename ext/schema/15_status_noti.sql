...
CREATE TABLE `status_noti` (
    `bk_st_id` INT(11) UNSIGNED NOT NULL,
    `status_id` INT(11) UNSIGNED NOT NULL,
    `notification_id` INT(11) UNSIGNED NOT NULL,
    `mail_send` BOOLEAN NOT NULL DEFAULT FALSE,   -- quick flag
    `sent_at` DATETIME DEFAULT NULL,              -- timestamp of send
    PRIMARY KEY (`bk_st_id`, `notification_id`),
    FOREIGN KEY (`bk_st_id`) REFERENCES `book_status`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
